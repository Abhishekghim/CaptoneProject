...
import { BarChart3, Download, FileDown, Loader2 } from "lucide-react";
...
export default function ReportsPanel() {
  ...
  // Each hook is its own independent fetch (house convention: no shared
  // cross-component cache) — this panel needs its own copies of the same
  // data AdminDashboard.tsx's other panels also fetch.
  const { data: scans, loadError: scansError } = useScans();
  const { data: billing, loadError: billingError } = useBilling();
  const { data: reports, loadError: reportsError } = useReports();
  const { data: profiles, loadError: profilesError } = useProfiles();
  const { data: equipment, loadError: equipmentError } = useEquipment();
  const { data: equipmentServiceLog, loadError: serviceLogError } = useEquipmentServiceLog();

  const loadError = scansError || billingError || reportsError || profilesError || equipmentError || serviceLogError;
  const loading =
    (!scans || !billing || !reports || !profiles || !equipment || !equipmentServiceLog) && !loadError;
  ...
  const equipmentUsage = useMemo(
    () =>
      (equipment ?? []).map((e) => ({
        ...
        service_events: (equipmentServiceLog ?? []).filter((l) => l.equipment_id === e.id).length,
      })),
    [equipment, equipmentServiceLog]
  );
